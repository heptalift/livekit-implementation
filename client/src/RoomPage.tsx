...
export const RoomPage = ({
  ...
  const {token, reset} = React.useContext(roomContext);
    ...
    let connect = async () => {
      ...
      if (connectionState === -1) {
        try {
          ...
          console.log('start audio session');
          await AudioSession.startAudioSession();
          console.log('connecting to room');
          await room.connect(livekitUrl, token, {});
          console.log('connected to ', livekitUrl, ' ', token);
  ...
  }, [reset, room, token, connectionState]);
